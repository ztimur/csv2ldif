...
def get_distinguish_name(uid, survey, org, dep):
    return u"cn={},ou={},ou={},ou={},ou=students,ou=users,dc=knu,dc=kg".format(uid, dep, org,survey)
...
def get_ou_list(survey):
    server = Server("ldap.knu.kg")
    conn = Connection(
        server, user="uid=ldap-user,dc=knu,dc=kg", password="q1wqgzk")
    conn.bind()
    conn.start_tls()
    result = conn.search('ou={},ou=students,ou=users,dc=knu,dc=kg'.format(survey),
...
def get_ou_distinguish_names(survey, org, dep):
    result = []
    result.append(u"ou={},ou=students,ou=users,dc=knu,dc=kg".format(survey))
    result.append(u"ou={},ou={},ou=students,ou=users,dc=knu,dc=kg".format(org, survey))
    result.append(
        u"ou={},ou={},ou={},ou=students,ou=users,dc=knu,dc=kg".format(dep, org,survey))
    return result
#    return []
...
def main():
    ...
    for row in i:
        ...
        ou_names = get_ou_distinguish_names(survey, org, dep)
        ...
        if ou_names[2] not in ou_list:
            print (u"{} -> {} not in LDAP. adding {}".format(dep,
                                                             org, ou_names[1]))
            ou_list.add(ou_names[2])
            ou_file.write(u"{}\n".format(ou_template.render(
                {'dn': encode(ou_names[2]), 'name': encode(dep)})))
        ...
        if (len(row) > 12 and len(row[12]) > 0):
            form = row[12]
        ...
        if (len(row) > 13 and len(row[13]) > 0):
            form = row[13]
            course = u"{}-{}".format(org, form)
            course_dn = u'cn={},ou=StudentEnrollment,ou=moodle,dc=knu,dc=kg'.format(
                course)
